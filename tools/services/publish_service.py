...
def make_release_zips(dump_logs: Mapping[LanguageFlavor, list[int]], font_formats: Sequence[FontFormat]) -> None:
    ...
    for language_flavor in options.LANGUAGE_FLAVORS:
        for font_format in font_formats:
            zip_file_path = path_define.RELEASES_DIR.joinpath(f'zfull-pixel-font-{language_flavor}-{font_format}-v{configs.VERSION}.zip')
            with ZipFile(zip_file_path, 'w') as file:
                ...
                for font_size in dump_logs[language_flavor]:
                    font_file_path = path_define.OUTPUTS_DIR.joinpath(f'Zfull-{language_flavor.upper()}-{font_size}px.{font_format}')
